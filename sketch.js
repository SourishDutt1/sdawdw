...
function draw() {
  ...
  if(gameState === play) {
    ...
    if(keyDown("space") && frameCount %4===0) {
      laser = createSprite(spaceShip.x,spaceShip.y - 70);
  ...
  gameState=win
    ...
    fill("white");
    textSize(40);
    text("YOU WON",canvas.width/2-400,canvas.height/2);
    text("You saved the planet",canvas.width/2-400,canvas.height/2+100);
    text("Your final score:"+score,canvas.width/2-400,canvas.height/2+200);
    text("  press 's' to start game.",canvas.width/2,canvas.height/2-10);
    if(keyDown("s")) {
      gameState = play;
